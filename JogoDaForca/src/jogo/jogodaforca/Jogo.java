...
import jogo.jogodaforca.utils.ConsoleUtils;
		...
		while (true) {
			...
			try {
				...
				if (letrasUsadas.contains(letra)) {
					throw new CaractereInvalidoException("Esta letra ja foi utilizada");
				}
				...
				if(palavra.possuiLetra(letra)) {
					...
					if (erros < MAX_ERROS) {
						...
						System.out.println("Voce errou! Voce ainda pode errar " + (MAX_ERROS - erros) + " vez(es)");
					}
				}
				...
				if (palavra.acertouPalavra()) {
					System.out.println("Parabens! Voce acertou a palavra completa: " + palavra.getPalavraOriginal());
				...
				if (erros == MAX_ERROS) {
					System.out.println("Voce perdeu o jogo! A palavra correta era " + palavra.getPalavraOriginal());
					System.out.println("Fim do Jogo!");
					break;
				}
